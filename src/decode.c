...
dbrew_decode_instruction(uint8_t* fp, Rewriter* r, bool* isTerminator)
{
    ...
    for (int i = 0; !success; i++)
    {
        ...
        if (!success && desc->encoding != OE_Invalid)
            continue;
        ...
        if (r->decInstrCount >= r->decInstrCapacity)
        {
            printf("decode buffer full (size: %d instrs)\n", r->decInstrCapacity);
            success = false;
            break;
        }
        ...
        switch (desc->encoding)
        {
            ...
            case OE_Invalid:
                initSimpleInstr(instr, desc->type);
                printf("Decoder error: unsupported opcode: %s\n", dbrew_stringify_bytes(fp, 15));
                break;
            default:
                assert(0);
        }
